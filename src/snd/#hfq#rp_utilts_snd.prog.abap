  ...
  TRY.
      CALL BADI lr_badi_utilts_snd->select_relevant_data
        ...
        EXCEPTIONS
          ...
          OTHERS             = 3.
*{   REPLACE        EN1K921210                                        1
*\      IF sy-subrc = 1 OR lt_calc_form_h IS INITIAL.
      IF sy-subrc = 1 OR lt_calc_form_h_tmp IS INITIAL.
*}   REPLACE
        "Keine Einträge in Tabelle &1 zu MaLo &2 ab Datum &3 gefunden.
*{   REPLACE        EN1K921210                                        3
*\        MESSAGE i001(/hfq/utilts_snd) WITH '/hfq/calc_form_h' <fs_malo>-low lv_keydate.
*\        EXIT.
        "Für Z40-Malos zulässig, kein Eintrag in den Tabellen; Nur bei Einzelselektion ok
        APPEND INITIAL LINE TO lt_calc_form_h_tmp ASSIGNING FIELD-SYMBOL(<fs_tmp>).
        <fs_tmp>-malo_extui = <fs_malo>-low.
        UNASSIGN <fs_tmp>.
*}   REPLACE
*{   REPLACE        EN1K921210                                        2
*\      ELSEIF line_exists( lt_calc_form_h[ calc_form_stat = /hfq/cl_dp_utilts=>gc_calcform_stat_z33 ] ) AND ( sy-subrc = 2 OR lt_calc_form IS INITIAL ).
      ELSEIF line_exists( lt_calc_form_h_tmp[ calc_form_stat = /hfq/cl_dp_utilts=>gc_calcform_stat_z33 ] ) AND ( sy-subrc = 2 OR lt_calc_form_tmp IS INITIAL ).
*}   REPLACE
...
IF p_mpar IS NOT INITIAL.
  ...
  ENDLOOP.
*{   INSERT         EN1K921210                                        4
  IF ls_serviceprovider IS INITIAL. "Nicht an der Melo; Malo prüfen
    SELECT SINGLE int_ui FROM euitrans
      INTO @lv_int_ui
      WHERE ext_ui = @so_malo-low.
    ...
    LOOP AT lt_all_services INTO ls_all_services
      ...
      ls_serviceprovider-service    = ls_all_services-service.
      ls_serviceprovider-date_from  = ls_all_services-service_start.
      ls_serviceprovider-ext_ui = so_malo-low.
    ENDLOOP.
